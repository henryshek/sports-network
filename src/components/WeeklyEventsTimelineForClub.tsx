...
interface WeeklyEventsTimelineForClubProps {
  club: Club
}

export default function WeeklyEventsTimelineForClub({ club }: WeeklyEventsTimelineForClubProps) {
  ...
  const getClubEventsForDate = (date: Date) => {
    return mockEvents.filter(event => {
      const eventDate = new Date(event.date)
      // Check if event date matches and event is related to club (if club has events array)
      return (
        eventDate.getDate() === date.getDate() &&
        eventDate.getMonth() === date.getMonth() &&
        eventDate.getFullYear() === date.getFullYear() &&
        (club.events?.some(e => e.id === event.id) || false)
      )
    })
  }
  ...
  // Get all club events for the week
  const allClubEventsThisWeek = mockEvents.filter(event => {
    const eventDate = new Date(event.date)
  ...
  return (
    <div className="bg-white rounded-lg border border-border p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold text-foreground">{club.name} - Weekly Events</h2>
      ...
      {allClubEventsThisWeek.length === 0 ? (
        <div className="text-center py-12">
          <p className="text-3xl mb-3">📭</p>
          <p className="text-muted">No events scheduled for {club.name} this week</p>
        </div>
      ) : (
        <>
          {/* Timeline Grid */}
          <div className="grid grid-cols-1 md:grid-cols-7 gap-4 mb-6">
            {weekDates.map((date, index) => {
              const dayEvents = getClubEventsForDate(date)
              const isToday =
          ...
          <div className="pt-6 border-t border-border">
            <p className="text-sm text-muted">
              Total events this week: <span className="font-semibold text-foreground">{allClubEventsThisWeek.length}</span>
            </p>
          </div>
        </>
      )}
